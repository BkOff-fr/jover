...
function App() {
  ...
  const updateOnScroll = useCallback(() => {
      ...
      // Experience section complex animation
      ...
      if (experienceSection && transitionImage && experienceTitle) {
        ...
        // Start animation when section is 200px below viewport (before it enters)
        const triggerPoint = windowHeight + 200;
        let progress = 0;
        
        if (sectionTop <= triggerPoint) {
          // Progress from 0 when section is 200px below to 1 when section is at center of viewport
          progress = Math.max(0, Math.min(1, (triggerPoint - sectionTop) / (triggerPoint + windowHeight * 0.5)));
        }
        
        // Debug log
        console.log('Experience animation progress:', progress, 'sectionTop:', sectionTop, 'triggerPoint:', triggerPoint);
        
        // Always ensure image is positioned correctly
        transitionImage.style.display = 'block';
        transitionImage.style.opacity = '1';
        
        // Transition image animation
        if (progress > 0 && progress <= 0.6) {
          // Phase 1: Scaling and rotation while in parallax position (0 to 0.6)
          const scale = 1 + (progress * 2.5); // Scale from 1 to 3.5
          const rotation = progress * 30; // Rotate from 0 to 30 degrees
          
          // Keep in original parallax position but with scaling and rotation
          transitionImage.style.position = 'absolute';
          transitionImage.style.top = '380%';
          transitionImage.style.right = '10%';
          transitionImage.style.transform = `translateY(${-(scrollTop * 0.1)}px) scale(${scale}) rotate(${rotation}deg)`;
          transitionImage.style.zIndex = '50';
          transitionImage.style.borderRadius = `${20 - (progress * 15)}px`;
          
        } else if (progress > 0.6) {
          // Phase 2: Fixed position and full screen scaling (0.6 to 1.0)
          const finalProgress = (progress - 0.6) / 0.4; // Normalize to 0-1
          const scale = 3.5 + (finalProgress * 8.5); // Scale from 3.5 to 12
          
          // Fix position to center and scale to fill screen
          transitionImage.style.position = 'fixed';
          transitionImage.style.top = '50%';
          transitionImage.style.left = '50%';
          transitionImage.style.transform = `translate(-50%, -50%) scale(${scale}) rotate(30deg)`;
          transitionImage.style.zIndex = '200';
          transitionImage.style.borderRadius = `${5 - (finalProgress * 5)}px`;
          
        } else {
          // Normal parallax state - no scaling or rotation
          transitionImage.style.position = 'absolute';
          transitionImage.style.top = '380%';
          transitionImage.style.right = '10%';
        ...
        // Title animation: letters come from bottom with staggered timing
    ...
    <div className={`App ${isDarkTheme ? 'dark-theme' : ''}`}>
      ...
      <main className="main-content">
        ...
        <section className="images-gallery">
          ...
          {/* L'image de transition - dernière image visible avant L'EXPÉRIENCE */}
